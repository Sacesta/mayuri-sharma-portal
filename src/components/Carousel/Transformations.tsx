  ...
  return (
    <>
      <Swiper
        ...
        // className="mySwiper"
        style={{ marginLeft: 0, marginRight: 0, overflow: "hidden" }}
      >
        {grpPod[category?.rank]?.map((podcast: Podcast, index: number) => {
          return (
            <SwiperSlide key={`${podcast._id}${category._id}`}>
              <li>
                <div className="home_story_img">
                  <PodcastMedia media={podcast?.thumbnail} />
                  <p className="home_story_number">{`# ${index + 1}`}</p>
